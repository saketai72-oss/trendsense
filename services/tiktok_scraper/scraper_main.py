...
def _trigger_ai_pipeline(video_id, url, video_data, top_comments):
    ...
    """
    # Bỏ qua gọi cloud webhook nếu được cấu hình chạy local offline
    trigger_webhook = os.getenv("TRIGGER_CLOUD_WEBHOOK", "true").lower() == "true"
    if not trigger_webhook:
        print(f"  [ℹ️] Biến TRIGGER_CLOUD_WEBHOOK = False. Bỏ qua gửi Webhook đám mây, video {video_id} sẽ được worker local xử lý sau.")
        return
    ...
    try:
        ...
        if resp.status_code == 202:
            print("  [🚀] Đã gửi sang Gemini AI Backend → Accepted")
            return
...
def main():
    ...
    if queue_only_mode:
        ...
        print("[⚡] Bỏ qua bước scraping và không khởi động browser.\n")
        driver = None
    else:
    ...
    for i, link in enumerate(links, 1):
                ...
                try:
                    ...
                    if is_blocked(driver):
                        if attempt < 3:
                            ...
                            continue
                        print("  [🚫] Bị block! Dừng cào bằng Selenium — sẽ thử fallback sang API.")
                        break
                    page_loaded = True
                    break
                except Exception as e:
        ...
        stats = {}
        if page_loaded and driver is not None:
            ...
            if clean_link in api_stats:
                ...
                if driver is not None and is_blocked(driver):
        ...
        if views <= 0:
            print("  [✂️] Bỏ qua: Views = 0 (video bị ẩn hoặc lỗi parse).")
            continue

...
│  embedding_svc.py │               │  Whisper + BLIP + OCR  │
└────────┬──────────┘               └──────────┬─────────────┘
         │                                     │
         └─────────────┬───────────────────────┘
                       │
...
- **Luồng xử lý:**
  1. Client gọi `GET /api/upload-url` (`{filename, content_type}`) → nhận `{video_id, upload_url, storage_path}`. Backend pre-insert video vào DB (`insert_user_video()`, `ai_status='user_pending'`).
...
- Nếu có `storage_url` → **Upload path** → Trend Alignment Score + Insights.
- Nếu chỉ có `url` → **Scraper path** → Engagement metrics + DB update.
...
- **Retrain:** GitHub Actions `weekly_train.yml` — mỗi Chủ Nhật 00:00 ICT, push model mới lên repo
  - Label: Top 20% theo `viral_velocity` = positive class
...
- **Model:** Google Gemini `text-embedding-004` (768-dim vectors)
- **Lưu:** Cột `embedding vector(768)` trong bảng `videos`, IVFFlat index (`vector_cosine_ops`, 100 lists)
...
- **Migration:** `core/db/migrations/001_pgvector.sql` — Enable pgvector extension + thêm cột `embedding vector(768)` + tạo IVFFlat index.
...
Steps:
  ...
  3. Install services/tiktok_scraper/requirements.txt
  4. Install Google Chrome (google-chrome-stable)
  5. python -m services.tiktok_scraper.scraper_main
Secrets: DATABASE_URL, MODAL_WEBHOOK_URL, GEMINI_WEBHOOK_URL, PROXY_LIST
Mục đích: Cào TikTok trending → Insert DB → Trigger AI pipeline (Gemini primary, Modal fallback)
```

### `weekly_train.yml` — Chủ Nhật 00:00 ICT
```
Cron: 0 17 * * 0 (UTC) = 00:00 ICT Monday
Runs on: ubuntu-latest
Python: 3.12
...
Steps:
  ...
  3. Install tiktok_scraper + ai_engine requirements (with --only-binary scikit-learn)
  4. Bước 1: python -m services.tiktok_scraper.scraper_main (cào stats mới)
  5. Bước 2: python -m services.ai_engine.train_model (train Random Forest)
  6. Bước 3: git commit + push data/models/rf_model.joblib + metrics.json [skip ci]
Secrets: DATABASE_URL
Mục đích: Retrain Random Forest model hàng tuần trên dữ liệu 14 ngày gần nhất
```

### `weekly_cleanup.yml` — Chủ Nhật 01:00 ICT
```
Cron: 0 18 * * 0 (UTC) = 01:00 ICT Monday
Runs on: ubuntu-latest
Python: 3.10
Steps:
  1. Checkout code
  2. Setup Python 3.10 + pip cache
  3. Install psycopg2-binary + python-dotenv
  4. python scripts/reset_viral_predictions.py
Secrets: DATABASE_URL
Mục đích: Reset viral_probability về 0 cho video cũ hơn 2 tuần (cho phép re-predict với model mới)
```
...
| `GEMINI_API_KEY` | `""` | Video analysis (Gemini 2.5 Flash) + Semantic Embeddings (text-embedding-004) |
...
│       ├── embedding_service.py   # pgvector semantic search (text-embedding-004)
  ...
  → _trigger_ai_pipeline():
      [Primary] POST /api/analyze-gemini
          ...
          → Redis Worker (gemini_engine.py):
              ...
              → embedding_service → text-embedding-004 → pgvector (non-blocking)
              → cleanup Gemini file
...
GitHub Actions (Chủ Nhật 00:00 ICT)
  ...
  → train_model.py:
      ...
      → Label: Top 20% viral_velocity = positive
...
```text
GitHub Actions (Chủ Nhật 01:00 ICT)

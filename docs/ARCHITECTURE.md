...
| AI & ML | Gemini 2.5 Flash, Gemini Embedding, OpenRouter, Groq, Whisper (int8), BLIP, EasyOCR, scikit‑learn (Random Forest) |
| GPU Serverless | Modal (T4/L4, persistent volume) - STRICTLY for user uploads |
...
└───────────────┬───────────────────────┘
                ...
                ▼                    ▼
        ┌─────────────┐      ┌─────────────┐      ┌─────────────┐
        │ Redis (RQ)  │      │  OpenRouter │      │   Modal     │
        │ gemini_jobs │      │  (Fallback  │      │ (Uploads    │
        └──────┬──────┘      │  Scraper)   │      │   Only)     │
               │             └──────┬──────┘      └──────┬──────┘
               ▼                    │                    │
        ┌─────────────┐             │             ┌─────────────┐
...
- Fallback path for scraper: OpenRouter text-only analysis → Groq. Modal is NO LONGER used for scraper fallback.
- User uploads: Presigned URLs → Supabase Storage → Modal GPU → Trend Alignment Score (Daily quotas enforced: Free 2/day, Pro 10/day via SePay).
...
| plan                    | VARCHAR   | `free` / `pro_49k` |
| status                  | VARCHAR   | `active` / `expired` |
| started_at              | TIMESTAMP | Subscription start time |
...
| plan                    | VARCHAR   | Plan being purchased |
| status                  | VARCHAR   | `pending` / `completed` |
| reference_code          | VARCHAR   | Unique code for SePay transfer |
...
|-------------------------|-----------|-------------|
| user_id                 | UUID (PK) | References users table |
| usage_date              | DATE (PK) | Date in GMT+7 |
| videos_analyzed         | INT       | Daily upload counter |
...
| `/api/upload-url`            | POST   | Generate presigned URL for user upload (enforces daily quota) |
...
| `/api/subscriptions/status`  | GET    | Get current plan & quota |
| `/api/subscriptions/create-payment` | POST | Create pending payment & reference code |
| `/api/subscriptions/sepay-webhook`  | POST | Webhook for SePay (secures via reference_code matching) |
...
- **OpenRouter & Groq** – fallback chain for scraper videos.
- **Modal GPU Pipeline** – STRICTLY for user uploads. Uses Whisper (audio), BLIP (frames), EasyOCR (screen text), then calls OpenRouter → Groq fallback.

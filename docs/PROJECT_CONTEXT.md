...
- AI‑powered video analysis (Gemini 2.5 Flash primary, OpenRouter fallback for scraper; Modal GPU strictly for user uploads)
- User video uploads with Trend Alignment Score (0–100, Python deterministic) and Daily Quotas (Free: 2/day, Pro: 10/day via SePay integration)
...
TikTok / User Upload → FastAPI Backend → Redis Queue (RQ) → AI Workers
                                                           ├─ Gemini 2.5 Flash (Primary Scraper)
                                                           ├─ OpenRouter → Groq (Scraper Fallback)
                                                           └─ Modal GPU (User Uploads Only: Whisper+BLIP+OCR+LLM)
                                           ↓
...
**Key design choices:**
- **Primary AI path:** Gemini 2.5 Flash (via Redis RQ worker) for scraper.
- **Scraper Fallback path:** OpenRouter text-only analysis (Modal is no longer used for scraper).
- **User uploads:** Presigned URLs → Supabase Storage → Modal GPU → Trend Alignment Score (with daily quota limits: 2 for Free, 10 for Pro).
- **Payments:** SePay webhook integration for Pro subscriptions without authentication (using reference_code for security).
...
| GPU Serverless | Modal (T4/L4, persistent volume for HF models) - STRICTLY for user uploads |
| Database | Supabase PostgreSQL + pgvector |
| Storage | Supabase Storage (S3‑compatible) |
| Payments | SePay Webhook Integration |
...
- **All providers require fallback** – Gemini → OpenRouter → Groq for scraper.
- **Modal is strictly for user uploads** – Do not route scraper traffic to Modal.
...
2. **User upload pipeline**:  
   Frontend → `GET /api/upload-url` → Check Daily Quota (2/day Free, 10/day Pro) → `PUT` to Supabase Storage → `POST /api/analyze` (rate‑limited) → Modal GPU → Trend Alignment Score + insights → Supabase Realtime updates frontend.
   *(Users can upgrade to Pro via SePay QR code payment which triggers a secure webhook using reference_code).*

3. **Weekly retraining** (Sunday 00:00 ICT):  
   Scrape fresh data → train Random Forest (top 15% viral velocity as positive) → commit `rf_model.joblib` → backends load new model on next request.
